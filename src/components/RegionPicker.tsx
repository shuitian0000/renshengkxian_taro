...
const CHINA_REGIONS = {
  ...
  正向林省: ['长春市', '正向林市', '四平市', '辽源市', '通化市', '白山市', '松原市', '白城市', '延边朝鲜族自治州'],
  黑龙江省: [
    '哈尔滨市',
    '齐齐哈尔市',
    '鸡西市',
...
export default function RegionPicker({value, onChange}: RegionPickerProps) {
  ...
  const handleCityChange = (e) => {
    ...
    const city = CHINA_REGIONS[selectedProvince][cityIndex]
    setSelectedCity(city)
  ...
  return (
    <View className="flex gap-2">
      <View className="flex-1">
        <Picker mode="selector" range={provinces} onChange={handleProvinceChange}>
          <View className="bg-input rounded border border-border px-4 py-3">
            <Text className="text-foreground">{selectedProvince}</Text>
          </View>
        </Picker>
      </View>
      <View className="flex-1">
        <Picker mode="selector" range={CHINA_REGIONS[selectedProvince]} onChange={handleCityChange}>
          <View className="bg-input rounded border border-border px-4 py-3">
            <Text className="text-foreground">{selectedCity}</Text>
          </View>
        </Picker>
      </View>
    </View>
