...
## 技术要点
- 八卦加载动画：使用CSS纯动画实现标准太极图，整体圆形背景白色（阳），右半圆金色（阴），上方添加金色小圆（阴鱼头部，在白色区域）、下方添加白色小圆（阳鱼头部，在金色区域）形成标准S形分界线，阳鱼眼（金色小点）位于左上白色区域中心（top: 25%, left: 25%），阴鱼眼（白色小点）位于右下金色区域中心（bottom: 25%, right: 25%），3秒旋转一周
...
- 全屏模式优化：chartHeight增加到700（横屏）/800（竖屏），ScrollView添加右侧padding 80px避免控制按钮遮挡K线图内容，确保年龄横轴和运势数轴完整显示
- 应用名称：统一使用"人生K线图谱"
